    ...
    String [] arr2 = str.split("");
        ...
        if(calc1.isPresent(calc1.arr)){
            if (calc1.romanSize(calc1.arr)){
                return calc1.romanOperation(calc1.arr, calc1.o);
            }
            if (!calc1.romanSize(calc1.arr)){
        ...
        if (!calc1.isPresent(calc1.arr)){
            ...
            if (calc1.arabParsing(calc1.arr)){
                if (calc1.arabSize(calc1.arr)){
                    return Integer.toString(calc1.arabOperation(calc1.arr, calc1.o));
                }
                if (!calc1.arabSize(calc1.arr)){
                    throw new throwsException("// превышен допустимый диапазон числа");
                }
...
class calc {
    ...
    public String romanOperation(String arr[], char o) throws throwsException {
        switch (o) {
            case ('+'):
    ...
    public int arabOperation (String[] arr, char o){
        switch (o){
            case ('+'):
